...
\item Show that 
	\eqref{eq:9-orig-diff}
	can be expressed as 
\begin{align}
	y(n)	 = x\brak{n} + x\brak{n-2},  \quad x(0) = y(0) = 1, y(n) \ge 0
	\label{eq:9-orig-diff-rev}
\end{align}
where 
\begin{align}
	x(n) &= a(n+1) \\
	y(n)	 &= b(n+1)
	\label{eq:9-orig-diff-rev-cond}
\end{align}
...
\item Find 
\begin{align}
	r(k) = x(k)*\sbrak{u(k)-u(k-n)}
\end{align}
and 
show that 
\begin{align}
	\sum_{k=1}^{n}a_k = r(n-1)
\end{align}
